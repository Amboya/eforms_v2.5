...
@section('content')
    ...
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0 text-dark text-orange text-uppercase">SUBSISTANCE BY BUSINESS UNITS: <span class="text-green">{{$category}}</span></h1>
        ...
        <div class="card">
            <div class="card-header">
                ...
                <form class="form-inline ml-3" method="post"
                      action="{{route('subsistence.invoices.business.units.search')}}">
                    @csrf
            ...
            <div class="card-body">
                <div class="table-responsive">
                    ...
                    <table id="example1" class="table m-0">
                        ...
                        @foreach( $business_units as $dir )
                            <tr>
                                <td>{{++$total_num}}</td>
                                <td>{{$dir->user_unit_description ?? "none"}}</td>
                                <td>{{$dir->user_unit_bc_code ?? "none"}}</td>
                                <td>
                                    @foreach( $list->where('business_unit_code', $dir->user_unit_bc_code) as $item )
                                        {{$item->total ?? 0}}
                                    @endforeach
                                </td>
                                <td>
...
@push('custom-scripts')
    ...
    <script>
        $(function () {
            ...
            var directorates = {!! json_encode($b_units) !!};
            var areaChartData = {
                labels: ['2021'],
                datasets: [
                    {
