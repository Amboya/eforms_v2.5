...
class ReportsController extends Controller
{
    ...
    public function units($status_id)
    {
        ...
        config_status_id, count(cost_center) as total, sum(total_payment) as amount ,sum(change) as change ,
        user_unit_code, business_unit_code ,cost_center, directorate_id
        from subsistence_totals_view
        ...
        //STATUS LIST
        $status = StatusModel::where('eform_id', config('constants.eforms_id.subsistence'))->get();
        ...
        //RETURN VIEW
        return view('eforms.subsistence.reports.units')->with(
    ...
    public function unitsSearch(Request $request)
    {
        ...
        elseif ($status_id == config('constants.money_given')) {
            $status_id_1 = config('constants.subsistence_status.funds_disbursement');
            $status_id_2 = config('constants.subsistence_status.funds_acknowledgement');
            $status_id_3 = config('constants.subsistence_status.security_approved');
            $status_id_4 = config('constants.subsistence_status.receipt_approved');
            $status_id_5 = config('constants.subsistence_status.closed');
            $status_id_6 = config('constants.subsistence_status.audited');
            $status_id_7 = config('constants.subsistence_status.reimbursement_box');
            $status_id_8 = config('constants.subsistence_status.await_audit');
            $status_id_9 = config('constants.subsistence_status.audit_box');
            $form = DB::select("SELECT
        count(cost_center) as total, sum(total_payment) as amount ,sum(change) as change ,
        user_unit_code, business_unit_code ,cost_center, directorate_id
        from subsistence_totals_view
        ...
        elseif ($status_id == config('constants.money_queried')) {
            $status_id_1 = config('constants.subsistence_status.audit_rejected');
            $form = DB::select("SELECT
       count(cost_center) as total, sum(total_payment) as amount ,sum(change) as change ,
        user_unit_code, business_unit_code ,cost_center, directorate_id
        from subsistence_totals_view
        ...
        elseif ($status_id == config('constants.money_pending')) {
            $status_id_1 = config('constants.subsistence_status.new_application');
            $status_id_2 = config('constants.subsistence_status.hod_approved');
            $status_id_3 = config('constants.subsistence_status.hr_approved');
            $status_id_4 = config('constants.subsistence_status.chief_accountant');
            $form = DB::select("SELECT
        count(cost_center) as total, sum(total_payment) as amount ,sum(change) as change ,
        user_unit_code, business_unit_code ,cost_center, directorate_id
        from subsistence_totals_view
        ...
        elseif ($status_id == config('constants.money_rejected')) {
            $status_id_1 = config('constants.subsistence_status.rejected');
            $status_id_2 = config('constants.subsistence_status.void');
            $status_id_3 = config('constants.subsistence_status.cancelled');
            $form = DB::select("SELECT
         count(cost_center) as total, sum(total_payment) as amount ,sum(change) as change ,
        user_unit_code, business_unit_code ,cost_center, directorate_id
        from subsistence_totals_view
    ...
    public function directorates($status_id)
    {
        ...
        //RETURN VIEW
        return view('eforms.subsistence.reports.directorates')->with(
            compact('list', 'category', 'totals_needs_me',
    ...
    public function directoratesSearch(Request $request)
    {
        ...
        * ***************************************/
        if ($status_id == config('constants.all')) {
            $form = DB::select("SELECT
        count(cost_center) as total, sum(total_payment) as amount ,sum(change) as change , directorate_id
        from subsistence_totals_view
        ...
        ");
            ...
            elseif ($request->date_from != null && $request->date_to != null) {
                ...
                $form = DB::select(
                    "SELECT count(cost_center)  as total, sum(total_payment) as amount ,sum(change) as change , directorate_id
                    FROM (
...
//                dd(5555);
    ...
    public function divisions($status_id)
    {
        ...
        //RETURN VIEW
        return view('eforms.subsistence.reports.divisions')->with(
            compact('list', 'category', 'totals_needs_me',
                'total_num', 'status', 'direc', 'directorates', 'date_range'
            ));
    }
        ...
        * ***************************************/
        ...
        elseif ($status_id == config('constants.money_given')) {
            ...
            elseif ($request->date_from != null && $request->date_to != null) {
                ...
                print_r("SELECT count(cost_center) as total, sum(total_payment) as amount ,sum(change) as change , division_id
                    FROM (
                ...
                $date_range = "From " . $request->date_from . " To " . $request->date_to;
                $form = DB::select(
                    "SELECT count(cost_center) as total, sum(total_payment) as amount ,sum(change) as change , division_id
                    FROM (
                         SELECT id,   config_status_id ,total_payment ,change , division_id
                         from subsistence_totals_view
                         where created_at BETWEEN  '{$request->date_from}' AND '{$request->date_to}'
    ...
    public function updateThePettyCashAccountsWithCorrectPTCODE($status_id)
    {
        ...
        $accounts_lines = DB::select("
            select * from subsistence_totals_view_account where id > 20000 and  id < 25000
        ");
        ...
        foreach ($accounts_lines_list as $accounts_line) {
            ...
            $subsistence_id = $accounts_line->subsistence_totals_view_id;
            $subsistence = DB::select("SELECT * FROM subsistence_totals_view
                            WHERE id = {$subsistence_id}  ");
            $tasks_pt = SubsistenceModel::hydrate($subsistence)->first();
            ...
            $accounts_line->subsistence_code = $tasks_pt->code;
    ...
    public function duplicates($status_id)
    {
        ...
        if ($status_id == config('constants.all')) {
            $form = DB::select("
            select * from subsistence_totals_view where code  in (  SELECT code
                FROM subsistence_totals_view
                GROUP BY code
                HAVING COUNT(code) > 1 )
        ");
        ...
        //RETURN VIEW
        return view('eforms.subsistence.reports.duplicates')->with(
            compact('list', 'category', 'totals_needs_me',
                'total_num', 'status', 'direc', 'directorates', 'date_range'
            ));
    }
    ...
    public function businessUnits($status_id)
    {
        ...
        //RETURN VIEW
        return view('eforms.subsistence.reports.business_units')->with(
            compact('list', 'category', 'totals_needs_me',
                'total_num', 'status', 'direc', 'directorates', 'date_range'
            ));
    }
        ...
        * ***************************************/
        ...
        ");
            ...
            if ($request->date_from == null && $request->date_to == null) {
                $date_range = "Cumulative Totals";
                $form = DB::select("SELECT
                    count(cost_center) as total, sum(total_payment) as amount ,sum(change) as change , business_unit_code
                    from subsistence_totals_view
...
//        dd($user_units);
    ...
    public function getFilteredReports($user_unit, $status, $start_date, $end_date)
    {
        ...
        if ($belongs_to_superior->exists()) {
            ...
            foreach ($belongs_to_superior as $item) {
                ...
                $list_one = DB::select("SELECT * FROM subsistence_totals_view
                    where config_status_id = '{$status}'
                      and user_unit_code = '{$user_unit_new}'
                      and  created_at <= '{$end_date}'
                      and  created_at >= '{$start_date}'
                     ");
                $my_list = SubsistenceModel::hydrate($list_one);
                if (sizeof($my_list) < 1) {
                    //dd($my_list);
                } else {
                    $list[] = SubsistenceModel::hydrate($list_one);
                }


                //get the summary
                $summary_one = DB::select("SELECT sum(amount) as amount , sum(total)as total
                    FROM subsistence_totals_view_dashboard_daily_totals_view
                      where config_status_id = '{$status}'
                      and  user_unit_code = '{$user_unit_new}'
            ...
            //get the list of transactions
            $list = DB::select("SELECT * FROM subsistence_totals_view
                    where config_status_id = '{$status}'
    ...
    function index(Request $request)
    {
        ...
        $dir = $directorate->where('config_status_id', config('constants.subsistence_status.closed'))->get();
        ...
        //reports one page
        return view('eforms.subsistence.reports.index')->with($params);
    }
    ...
    function syncDirectorates()
    {
        ...
        $total_forms = DB::select("SELECT SUM(total_payment) as amount,  count('id') as total , directorate_id
           FROM subsistence_totals_view where config_status_id = {$closed_status} group by directorate_id  order by amount desc ");
        $total_forms = SubsistenceModel::hydrate($total_forms)->all();
        ...
        $total_forms = DB::select("SELECT SUM(total_payment) as amount,  count('id') as total , directorate_id
           FROM subsistence_totals_view where config_status_id = {$new_status} group by directorate_id  order by amount desc ");
        $total_forms = SubsistenceModel::hydrate($total_forms)->all();
        ...
        $total_forms = DB::select("SELECT SUM(total_payment) as amount,  count('id') as total , directorate_id
           FROM subsistence_totals_view where config_status_id = {$rejected_status} group by directorate_id  order by amount desc ");
        $total_forms = SubsistenceModel::hydrate($total_forms)->all();
        ...
        $total_forms = DB::select("SELECT SUM(total_payment) as amount,  count('id') as total , directorate_id
           FROM subsistence_totals_view where config_status_id = {$cancelled_status} group by directorate_id  order by amount desc ");
        $total_forms = SubsistenceModel::hydrate($total_forms)->all();
        ...
        $void_forms = DB::select("SELECT SUM(total_payment) as amount,  count('id') as total , directorate_id, user_unit_code
           FROM subsistence_totals_view where config_status_id = {$void_status} group by directorate_id , user_unit_code order by amount desc ");
        $total_forms = SubsistenceModel::hydrate($void_forms)->all();
    ...
    function syncUserUnits()
    {
        ...
        $total_forms = DB::select("SELECT SUM(total_payment) as amount,  count('id') as total , directorate_id, user_unit_code
           FROM subsistence_totals_view where config_status_id = {$closed_status} group by directorate_id , user_unit_code order by amount desc ");
        $total_forms = SubsistenceModel::hydrate($total_forms)->all();
        ...
        $new_status = config('constants.subsistence_status.new_application');
        $total_forms = DB::select("SELECT SUM(total_payment) as amount,  count('id') as total , directorate_id, user_unit_code
           FROM subsistence_totals_view where config_status_id = {$new_status} group by directorate_id , user_unit_code order by amount desc ");
        $total_forms = SubsistenceModel::hydrate($total_forms)->all();
        ...
        $rejected_status = config('constants.subsistence_status.rejected');
        $total_forms = DB::select("SELECT SUM(total_payment) as amount,  count('id') as total , directorate_id, user_unit_code
           FROM subsistence_totals_view where config_status_id = {$rejected_status} group by directorate_id , user_unit_code order by amount desc ");
        $total_forms = SubsistenceModel::hydrate($total_forms)->all();
        ...
        $cancelled_status = config('constants.subsistence_status.cancelled');
        $total_forms = DB::select("SELECT SUM(total_payment) as amount,  count('id') as total , directorate_id, user_unit_code
           FROM subsistence_totals_view where config_status_id = {$cancelled_status} group by directorate_id , user_unit_code order by amount desc ");
        $total_forms = SubsistenceModel::hydrate($total_forms)->all();
