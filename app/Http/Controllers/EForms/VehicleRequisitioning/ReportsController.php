...
use \App\Http\Controllers\Eforms\VehicleRequisitioning\HomeController;
...
//        dd($user_units);
    ...
    if ($user->type_id == config('constants.user_types.developer')) {
        ...
        $status = StatusModel::where('eform_id', config('constants.eforms_id.vehicle_requisitioning'))->orderBy('name')->get();
        ...
        //reports one page
        return view('eforms.vehicle-requisitioning.reports.filtered_reports')->with($params);
    }
      ...
      //  dd($belongs_to_superior->get());
        if($belongs_to_superior->exists()){
            ...
            foreach ($belongs_to_superior as $item){
                ...
                $list_one = DB::select("SELECT * FROM eform_vehicle_requisitioning
                    where config_status_id = '{$status}'
                      and user_unit_code = '{$user_unit_new}'
                      and  created_at <= '{$end_date}'
                      and  created_at >= '{$start_date}'
                     ");
                $my_list = VehicleRequisitioningModel::hydrate($list_one);
                if(sizeof($my_list) < 1){
                    //dd($my_list);
                }else{
                    $list[] = VehicleRequisitioningModel::hydrate($list_one);
                }
                ...
                $summary_one = DB::select("SELECT sum(amount) as amount , sum(total)as total
                    FROM eform_vehicle_requisitioning_dashboard_daily_totals_view
                      where config_status_id = '{$status}'
                      and  user_unit_code = '{$user_unit_new}'
    ...
    function index(Request $request)
    {
        ...
        $dir =  $directorate ->where('config_status_id',  config('constants.vehicle_requisitioning_status.closed') )->get() ;
       ...
       // dd($unit22);
        ...
        //reports one page
        return view('eforms.vehicle-requisitioning.reports.index')->with($params);
    }
    ...
    function syncDirectorates()
    {
         ...
         * total Closed
         */
        $closed_status = config('constants.vehicle_requisitioning_status.closed');
        $total_forms = DB::select("SELECT SUM(total_payment) as amount,  count('id') as total , directorate_id
           FROM eform_vehicle_requisitioning where config_status_id = {$closed_status} group by directorate_id  order by amount desc ");
        $total_forms = VehicleRequisitioningModel::hydrate($total_forms)->all();

        // dd($total_forms);

        foreach ($total_forms as $total) {
            $total_create = Totals::updateOrCreate([
                'eform_id' => config('constants.eforms_id.vehicle_requisitioning'),
                'eform_code' => config('constants.eforms_name.vehicle_requisitioning'),
        ...
        $total_forms = DB::select("SELECT SUM(total_payment) as amount,  count('id') as total , directorate_id
           FROM eform_vehicle_requisitioning where config_status_id = {$new_status} group by directorate_id  order by amount desc ");
        $total_forms = VehicleRequisitioningModel::hydrate($total_forms)->all();
         ...
         * total rejected
         */
        $rejected_status = config('constants.vehicle_requisitioning_status.rejected');
        $total_forms = DB::select("SELECT SUM(total_payment) as amount,  count('id') as total , directorate_id
           FROM eform_vehicle_requisitioning where config_status_id = {$rejected_status} group by directorate_id  order by amount desc ");
        $total_forms = VehicleRequisitioningModel::hydrate($total_forms)->all();
         ...
         * total pending
         */
        $status1 = config('constants.vehicle_requisitioning_status.hod_approved');
        $status2 = config('constants.vehicle_requisitioning_status.hr_approved');
        $status3 = config('constants.vehicle_requisitioning_status.chief_accountant');
        $status4 = config('constants.vehicle_requisitioning_status.funds_disbursement');
        $status5 = config('constants.vehicle_requisitioning_status.funds_acknowledgement');
        $status6 = config('constants.vehicle_requisitioning_status.security_approved');
        $total_forms = DB::select("SELECT SUM(total_payment) as amount,  count('id') as total , directorate_id
           FROM eform_vehicle_requisitioning
         ...
         * total Cancelled
         */
        $cancelled_status = config('constants.vehicle_requisitioning_status.cancelled');
        $total_forms = DB::select("SELECT SUM(total_payment) as amount,  count('id') as total , directorate_id
           FROM eform_vehicle_requisitioning where config_status_id = {$cancelled_status} group by directorate_id  order by amount desc ");
        $total_forms = VehicleRequisitioningModel::hydrate($total_forms)->all();
         ...
         * total Void
         */
        $void_status = config('constants.vehicle_requisitioning_status.void');
        $void_forms = DB::select("SELECT SUM(total_payment) as amount,  count('id') as total , directorate_id, user_unit_code
           FROM eform_vehicle_requisitioning where config_status_id = {$void_status} group by directorate_id , user_unit_code order by amount desc ");
        $total_forms = VehicleRequisitioningModel::hydrate($void_forms)->all();
    ...
    function syncUserUnits()
    {
        ...
        $total_forms = DB::select("SELECT SUM(total_payment) as amount,  count('id') as total , directorate_id, user_unit_code
           FROM eform_vehicle_requisitioning where config_status_id = {$closed_status} group by directorate_id , user_unit_code order by amount desc ");
        $total_forms = VehicleRequisitioningModel::hydrate($total_forms)->all();
        ...
        $new_status = config('constants.vehicle_requisitioning_status.new_application');
        $total_forms = DB::select("SELECT SUM(total_payment) as amount,  count('id') as total , directorate_id, user_unit_code
           FROM eform_vehicle_requisitioning where config_status_id = {$new_status} group by directorate_id , user_unit_code order by amount desc ");
        $total_forms = VehicleRequisitioningModel::hydrate($total_forms)->all();
        ...
        $rejected_status = config('constants.vehicle_requisitioning_status.rejected');
        $total_forms = DB::select("SELECT SUM(total_payment) as amount,  count('id') as total , directorate_id, user_unit_code
           FROM eform_vehicle_requisitioning where config_status_id = {$rejected_status} group by directorate_id , user_unit_code order by amount desc ");
        $total_forms = VehicleRequisitioningModel::hydrate($total_forms)->all();
        ...
        $cancelled_status = config('constants.vehicle_requisitioning_status.cancelled');
        $total_forms = DB::select("SELECT SUM(total_payment) as amount,  count('id') as total , directorate_id, user_unit_code
           FROM eform_vehicle_requisitioning where config_status_id = {$cancelled_status} group by directorate_id , user_unit_code order by amount desc ");
        $total_forms = VehicleRequisitioningModel::hydrate($total_forms)->all();
